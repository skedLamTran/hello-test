...
const activateFocusTrap = ({
  ...
  const getInitialFocusNodeByPriority = () => {
    ...
    const tabbableInputElement = inputElements.find(inputElement => (0, _tabbable.isTabbable)(inputElement));
    ...
    const firstTabbableFooterElement = footerElement ? (0, _tabbable.tabbable)(footerElement)[0] : null;

    if (firstTabbableFooterElement) {
      return firstTabbableFooterElement;
    }

    const firstTabbableElement = (0, _tabbable.tabbable)(element)[0];
  ...
  const getInitialFocusNode = () => {
    ...
    if (!node || !(0, _tabbable.isFocusable)(node)) {
  ...
  const updateTabbableNodes = () => {
    const tabbableNodes = (0, _tabbable.tabbable)(element);
  ...
  const checkPointerDown = function checkPointerDown(e) {
    if (!element.contains(e.target) && options !== null && options !== void 0 && options.clickOutsideDeactivates) {
      deactivate();
    }
  };

  const checkFocusIn = e => {
    if (element.contains(e.target) || e.target instanceof Document) {
      return;
    }
